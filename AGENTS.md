...
> **Honest scope of the firewall.** NB's evidence logs (`.nb/logs/`) are **tamper-evident, not tamper-proof.** `/nb:close` reconciles every "done" against records the AI did not merely narrate — the hook-written tool log **or nb-run's trusted-execution log** (`run_id` + `output_sha256` from the real run), plus a hash-bound cross-family review (artifact body hash == provenance == hook-logged hash). That stops accidental and agentic overclaim; it is **not** a cryptographic attestation system against a malicious user with write access to `.nb/`. Review-coverage ("analytical") proofs are **provenance-bound, not correctness-proven**: NB verifies the review genuinely ran and is unmodified, never that its conclusions are right. The **core review** carries the same provenance check: `/nb:close` labels it `cross-family` (machine-bound to a real `cross-review.mjs` run) or `manual` (Generic-mode / hand-pasted — low strength, surfaced as a warning, never silently credited as cross-family); a strict preset (`cross_family_required`) makes the machine-bound review a hard gate. The **design (GATE-1) review is one-way / advisory** — what close enforces at the design stage is the 3-tier docs' *existence* (`validate-state`), not that a plan review ran. `approved_by` on a decision is **accountability friction, not proof a human approved.**
